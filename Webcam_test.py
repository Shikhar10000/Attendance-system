        ...
        with sess.as_default():
            ...
            if runt==1:
                for a in range(1,len(HumanNames)):
                    ...
                    db.child("Attendance").child(now.year).child(now.month).child(now.day).child(now.hour).update(student, user['idToken'])
                    ...
                    if nrof_faces > 0:
                        ...
                        for i in range(nrof_faces):
                            ...
                            if best_class_probabilities>0.43:
                                ...
                                text_x = bb[i][0]
                                text_y = bb[i][3] + 20
                                print('Result Indices: ', best_class_indices[0]+1)
                                print(HumanNames)
                                now = datetime.datetime.now()
                                print("printing to firebase")
                                student = { HumanNames[best_class_indices[0]+1] : "Present" }
                                db.child("Attendance").child(now.year).child(now.month).child(now.day).child(now.hour).update(student, user['idToken'])
